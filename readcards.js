...
const getCardName = function (cardName) {
  ...
  return fileNameSplited.length > 1 ? fileNameSplited.pop() : fileNameSplited[0]
}
...
const getCard = (content, parent) => {
  if (content.extension == 'txt') {
    ...
    const cardType =
      formatCardName(parent.name).toLowerCase() == formatCardName(content.name).toLowerCase()
        ? 'cover'
        : 'card'
    const cardImage =
      findCardFile(content.name, parent, 'jpg') || findCardFile(content.name, parent, 'png')
    const cardAudio =
      findCardFile(content.name, parent, 'mp3') || findCardFile(content.name, parent, 'mpeg')
...
const findCardFile = (name, parent, extension) => {
  ...
  if (parent && parent.contents)
    parent.contents.forEach((content) => {
      if (content.name === name && content.extension === extension) file = content
    })
